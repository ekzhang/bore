...
* `bore local` - deployment on the cluster and routing traffic to
  * `6443` - the Kubernetes API server
  * `2222` - for SSH
  * `80` and `443` - the Ingress Controller
...
* Create the `Secret` to store `BORE_SECRET`
...
* Create the `ConfigMap` with your current public IP address
...
* Create the `ServiceAccount` that the job will use
...
* Create the `Role` that can restart the `Pod` and update the `ConfigMap` in the namespace
...
* Configure the Git `Service` with a `NodePort`
